...
`, "peer-a", 100, 200, 0, 0, oldTime, "peer-a", 150, 260, 50, 60, newTime)
	require.NoError(t, err)

	totals, err := database.LatestPeerTotals()
	require.NoError(t, err)
	require.Equal(t, uint64(150), totals["peer-a"].Rx)
	require.Equal(t, uint64(260), totals["peer-a"].Tx)

	samples, err := database.TrafficSamples("peer-a", oldTime.Add(time.Minute))
	require.NoError(t, err)
